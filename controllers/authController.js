...
exports.enviarToken = async (req, res) => {
    ...
    if(!email) {
        req.flash('error', 'El correo no puede ir vacio');
    ...
    usuario.expiracion = Date.now() + (36000 * 4);
...
exports.actualizarPassword = async (req, res) => {
    ...
    if(req.body.password.length === 0) {
        req.flash('error', 'La contraseña no debe de ir vacia');
        return res.redirect('/reestablecer/' + req.params.token);
    }
    if(req.body.password.length < 6) {
        req.flash('error', 'La contraseña debe contener al menos 6 caracteres');
        return res.redirect('/reestablecer/' + req.params.token);
    }
